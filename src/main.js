...
ipcMain.handle('save-new-template', async (event, { tempPath, configData }) => {
        try {
            ...
            // Simpan config awal (termasuk isFree)
            templateConfigs[fileName] = {
                path: newPath,
                visible: true,
                metadata: { width: metadata.width, height: metadata.height },
                positions: configData.positions || [],
                price: configData.price || 15000,
                isFree: configData.isFree || false // [BARU] Default false
            };
            saveConfig();
    ...
    // [BARU] Handler untuk menyimpan video sementara (Live Moment)
    ipcMain.handle('save-temp-video', async (event, { buffer, index }) => {
        try {
            // Buat folder temp_videos jika belum ada
            const videoDir = getAppPath('temp_videos');
            if (!fs.existsSync(videoDir)) fs.mkdirSync(videoDir);

            const fileName = `live-clip-${Date.now()}-${index}.webm`;
            ...
            console.error("Gagal simpan video:", error);
            return { success: false, error: error.message };
        }
    });
...
ipcMain.handle('update-template-config', async (event, { fileName, configData }) => {
        if (templateConfigs[fileName]) {
            // Update posisi, visibility, harga, DAN STATUS GRATIS
            ...
            if (configData.isFree !== undefined) templateConfigs[fileName].isFree = configData.isFree; // [BARU]
